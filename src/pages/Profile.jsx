        ...
        <div className="tw-mt-28 tw-flex tw-flex-wrap tw-justify-between tw-items-center">
          ...
          </div>
          {
            props.Posts.filter(post => post.user.id === parseInt(params.id))[0] !== undefined ? (
              <img onClick={() => navigate(`/posts/${props.Posts.filter(post => post.user.id === parseInt(params.id))[0].id}`)} src={props.Posts.filter(post => post.user.id === parseInt(params.id))[0].image_1} alt="Latest Post" className="animate__animated animate__zoomIn tw-w-full md:tw-w-6/12 tw-cursor-pointer" />
            ) : (
